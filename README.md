...
├── express-server/     # Тестовый сервер для mock-данных
...
В сборке реализован лёгкий тестовый backend на Express для отдачи mock-данных из JSON-файлов.

Запуск сервера командой

```
"express":    "npx nodemon ./express-server/server.js"
```

Либо исключительно сервера,

```
"express":    "npx nodemon ./express-server/server.js"
```

- Порт по умолчанию — **5000** (константа `PORT`).
- Включены CORS и автоматический парсинг JSON-тел запросов.
- При старте сканируется папка `express-server/db` на файлы `*.json`.
- Для каждого файла `<name>.json` автоматически создаются два маршрута:

  - `GET /api/<name>` — возвращает весь JSON из файла.
  - `GET /api/<name>/:id` — ищет в массиве объект с указанным `id` и возвращает его (404, если не найден).

- При ошибках чтения или парсинга JSON возвращается статус **500**.
