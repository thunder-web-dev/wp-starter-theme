...
Любые изображения, кроме **favicon**, клади в папку **img** (`assets/src/img`).

По умолчанию все **jpg/jpeg/png** из `assets/src/img` при сборке **автоматически конвертируются в `.webp`** и попадают в `assets/build/img` (исходные `.jpg/.png` в `build` не копируются).

Если нужно **сохранить исходный формат** (оставить `.jpg/.png`), добавь суффикс **`_keep`** перед расширением — такие файлы **не конвертируются** и копируются в `assets/build/img` как есть:

- `hero.jpg` → `hero.webp`
- `hero_keep.jpg` → `hero_keep.jpg`

Если нужно сделать svg-спрайт, клади нужные для спрайта svg-файлы в папку **img/svg** (`assets/src/img/svg`). При этом, такие атрибуты как `fill`, `stroke`, `style` будут автоматически удаляться. Иные svg-файлы просто оставляй в `assets/src/img` — они копируются в `assets/build/img` как есть.

Подключать **webp** лучше через тег `picture`. Для `background-image` можно использовать `image-set` там, где это [возможно](https://caniuse.com/css-image-set).
